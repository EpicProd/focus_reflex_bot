...
from focus_reflex.core.config_manager import ConfigManager
from focus_reflex.core.loader import ModuleLoader
from focus_reflex.core.task_manager import (
    BaseCoreTask,
    BaseSchedulerTask,
    TaskManager,
)
...
class BotCore:
    ...
    async def _startup(self, dispatcher: Dispatcher):
        ...
        if self.config.get_item("features", "use_modern_callback"):
            from focus_reflex.keyboards.middlewares import (  # pylint: disable=import-outside-toplevel
        ...
        try:
            from focus_reflex.keyboards.models.base import (  # pylint: disable=import-outside-toplevel
                ButtonRegistry,
            )
        ...
        if self.config.get_item("features", "use_database"):
            from focus_reflex import (  # pylint: disable=import-outside-toplevel
                db,
            )
