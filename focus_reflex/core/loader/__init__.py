...
class ModuleLoader:
    ...
    def __init__(
        self, project_name: str, is_prod: bool, config: ConfigManager
    ) -> None:
        logger.info(f"Initializing {project_name} modloader...")
        self.project_name = project_name
        self.is_prod = is_prod
        self.config = config
        self.supported_modules = self.config.get_item(
    ...
    def __list_all_modules(self, abs_path: str) -> List[str]:
        ...
        all_dir_modules = [
            basename(f[:-1])
            for f in dir_mod_paths
            if isdir(f[:-1])
            and not f[:-1].endswith(".py/")
    ...
    def get_loadable(self) -> List[str]:
        ...
        return [
            x
            for x in self.supported_modules
