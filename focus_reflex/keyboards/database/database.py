...
try:
    ...
    from focus_reflex.keyboards.database.models import Base, Callback
except ImportError:
    Base = None

from focus_reflex import config, loop
    ...
    async def init_database():
        if not Path("static").exists():
            ...
            if not (
                ...
                logger.error(
                    "static directory not available on a host machine!"
                )
                ...
                callback: Callback = (
                    (
                        await session.execute(
                            select(Callback).filter_by(query=data)
                        )
                    )
                    .unique()
                    .scalar_one()
